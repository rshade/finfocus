...
func (s *DismissalStore) Set(record *DismissalRecord) error {
	...
	s.dismissals[record.RecommendationID] = record
	return nil
}
...
// This is used for the --include-dismissed view.
...
// CleanExpiredSnoozes removes all snoozed records whose ExpiresAt has passed.
// This implements auto-unsnooze at query time.
func (s *DismissalStore) CleanExpiredSnoozes() error {
	s.mu.Lock()
	defer s.mu.Unlock()
