...
func (b *BudgetsConfig) Validate() ([]string, error) {
	...
	// Validate provider budgets
	for name, provider := range b.Providers {
		if validErr := provider.Validate(globalCurrency); validErr != nil {
			return nil, fmt.Errorf("provider %q budget: %w", name, validErr)
		}
	}

	// Validate tag budgets and check for duplicate priorities
