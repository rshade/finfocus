...
func TestCalculatePluginTTL(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expiresAt:  timePtr(time.Now().Add(14 * 24 * time.Hour)), // 14 days
			defaultTTL: defaultTTL,
		...
		},
		{
			...
			expiresAt:  timePtr(time.Now().Add(time.Duration(MaxTTLSeconds) * time.Second)),
			defaultTTL: defaultTTL,
			wantTTL:    MaxTTLSeconds,
			wantSkip:   false,
			wantCapped: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotTTL, gotSkip, gotCapped := CalculatePluginTTL(tt.expiresAt, tt.defaultTTL)
