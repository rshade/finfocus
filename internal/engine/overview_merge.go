...
// MergeResourcesForOverview builds a combined list of OverviewRow entries from the current
// state resources and pending plan steps for display in an overview.
//
// The resulting slice preserves the order of stateResources for existing resources and
// appends new resources that exist only in the plan. For each custom resource found in the
// state, the function includes a row initialized from the state and then may update that
// row's status, property diffs, and projected properties if corresponding plan data exists.
// For plan-only create steps, the function appends rows initialized with the plan's URN and
// type, sets the status to creating, and populates projected and base properties when
// available. When multiple plan steps reference the same URN, deterministic precedence
// rules are used to resolve which step determines the row's status and which provides
// property diffs.
//
// Parameters:
//   ctx: the context for logging and cancellation.
//   stateResources: the list of resources currently recorded in state; only custom
//     resources are included in the result.
//   planSteps: the list of pending plan steps that may modify existing resources or
//     create new ones.
//
// Returns:
//   A slice of merged OverviewRow entries reflecting the current state enriched with
//   applicable plan-derived status, property diffs, and projected properties, and an
//   error if merging fails.
...
// this call.
//
// ApplyChangesToRows updates the Status field of any OverviewRow in rows whose URN exists in statusByURN.
// It modifies the slice elements in place. If rows is nil the function does nothing.
// Rows with no matching URN in statusByURN are left unchanged.
...
//
// ApplyPropertyDiffsToRows updates each OverviewRow in rows with the PropertyDiffs found in diffsByURN.
// For each row whose URN exists as a key in diffsByURN, the row's PropertyDiffs field is replaced with a copy
// of the corresponding slice to avoid aliasing the original slice.
// If rows is nil the function does nothing.
...
//
// ApplyProjectedPropertiesToRows applies projected properties to matching overview rows in place.
// For each row whose URN exists in projectedPropsByURN, sets row.ProjectedProperties to a deep
// clone of the corresponding map. If rows is nil the function is a no-op. The cloning prevents
// callers from retaining references to the original maps.
...
// the original backing arrays; URNs with no property diffs are not present in the map.
...
//   A map where keys are URNs and values are cloned projected property maps for the first non-empty projection seen for each URN.
...
// cloneProperties creates a deep copy of the provided map[string]interface{}.
// It recursively clones nested maps and slices (using cloneAny) so the returned
// map shares no mutable substructure with the input. If the input is empty,
// cloneProperties returns nil.
...
// cloneAny recursively clones supported composite values.
// It produces a deep copy for values of type map[string]interface{} and []interface{},
// allocating new maps and slices and cloning their elements/entries. For other types
// (including nil and primitive values), it returns the value unchanged. The returned
// value has the same dynamic type as the input.
