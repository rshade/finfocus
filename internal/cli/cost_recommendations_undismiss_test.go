...
// findUndismissSubcommand finds the undismiss subcommand.
func findUndismissSubcommand(cmd *cobra.Command) *cobra.Command {
	for _, sub := range cmd.Commands() {
		if sub.Name() == "undismiss" {
			return sub
		}
	}
	return nil
...
func TestNewRecommendationsUndismissCmd(t *testing.T) {
	...
	undismissSub := findUndismissSubcommand(cmd)
