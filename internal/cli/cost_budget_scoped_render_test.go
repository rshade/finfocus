...
func TestRenderProviderSection_SortedOutput(t *testing.T) {
	...
	awsIdx := len(output) - len(output[findIndex(output, "AWS"):])
	azureIdx := len(output) - len(output[findIndex(output, "AZURE"):])
	gcpIdx := len(output) - len(output[findIndex(output, "GCP"):])
