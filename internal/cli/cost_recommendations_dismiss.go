...
func executeDismiss(cmd *cobra.Command, recommendationID string, params dismissParams) error {
	...
	if !params.force {
		...
		if !confirmPrompt(cmd, "Continue? [y/N]: ") {
			cmd.PrintErrln("Dismissed cancelled.")
			return nil
		}
	}
...
func loadDismissalStore() (*config.DismissalStore, error) {
	...
	storePath := filepath.Join(homeDir, ".finfocus", "dismissed.json")
	...
	if loadErr := store.Load(); loadErr != nil {
		if errors.Is(loadErr, config.ErrStoreCorrupted) {
			return nil, fmt.Errorf("dismissal state file is corrupted; "+
				"remove or fix %s to continue: %w", storePath, loadErr)
		}
...
func createDismissEngine(
	...
	if planPath == "" {
		...
		return engine.New(nil, nil), nil, nil
	}
	...
	return engine.New(clients, nil), cleanup, nil
}
...
// confirmPrompt displays a prompt and reads user confirmation from stdin.
...
// mustParseDismissalReason parses a reason string, returning zero on error.
...
func mustParseDismissalReason(reason string) pbc.DismissalReason {
	...
	if err != nil {
		// This should never happen since we validate before calling
		return 0
	}
	return r
}
