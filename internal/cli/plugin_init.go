...
// generated. Docker files are generated by default; they are skipped when
// --minimal or --no-docker is set, or when --with-docker=false is passed.
// --docker-only always generates Docker files (it targets existing projects).
func (o *PluginInitOptions) ShouldGenerateDocker() bool {
	if o.DockerOnly {
		return true
	}
	if o.Minimal || o.NoDocker {
		return false
	}
	return o.WithDocker
}

// ShouldGenerateDocs reports whether the docs/ directory should be generated.
// Docs are generated by default; they are skipped when --minimal or --no-docs
// is set, or when --with-docs=false is passed.
func (o *PluginInitOptions) ShouldGenerateDocs() bool {
	if o.Minimal || o.NoDocs {
		return false
	}
	return o.WithDocs
}

// ShouldGenerateHealth reports whether the health endpoint code should be
// included in the generated main.go. Health support is generated by default;
// it is skipped when --minimal or --no-health is set, or when
// --with-health=false is passed.
func (o *PluginInitOptions) ShouldGenerateHealth() bool {
	if o.Minimal || o.NoHealth {
		return false
	}
	return o.WithHealth
}
...
- Example tests`,
  ...
  finfocus plugin init my-plugin --author "Your Name" --providers aws --output-dir /path/to/plugins`,
	...
	cmd.Flags().BoolVar(&opts.NoDocker, "no-docker", false,
		"Skip Docker file generation (alias for --with-docker=false)")
	cmd.Flags().BoolVar(&opts.DockerOnly, "docker-only", false,
		"Generate only Docker files (for adding to an existing project)")
	cmd.Flags().BoolVar(&opts.WithDocker, "with-docker", true, "Generate Docker support files")
	cmd.Flags().BoolVar(&opts.WithDocs, "with-docs", true, "Generate documentation templates")
	cmd.Flags().BoolVar(&opts.WithHealth, "with-health", true, "Include health endpoint code")
	cmd.Flags().BoolVar(&opts.Minimal, "minimal", false,
		"Generate minimal scaffold only (overrides --with-* flags)")
	cmd.Flags().BoolVar(&opts.NoDocs, "no-docs", false,
...
// would create, honoring the generation-control options.
