...
func TestBudgetsConfig_Helpers(t *testing.T) {
	...
	t.Run("GetEffectiveExitOnThreshold", func(t *testing.T) {
		...
		assert.True(t, cfg.GetEffectiveExitOnThreshold(ptr(true)))
	...
	t.Run("GetEffectiveExitCode", func(t *testing.T) {
		...
		cfgZero := &config.BudgetsConfig{ExitCode: ptr(0)}
		assert.Equal(t, 0, cfgZero.GetEffectiveExitCode(nil)) // Explicit 0 is respected

		cfgUnset := &config.BudgetsConfig{ExitCode: nil}
		assert.Equal(t, 1, cfgUnset.GetEffectiveExitCode(nil)) // Nil defaults to 1
