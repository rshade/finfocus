...
- `make lint`: Run `golangci-lint` v2.13.2 plus `markdownlint`.
- `make validate`: Run `go mod tidy -diff` and `go vet` checks.
- `make docs-lint`: Lint docs when editing Markdown.

### Go Formatting and Imports

- Go 1.27.1+; use tabs for indentation and run `gofmt` on Go files.
...
- Go 1.27.1 + finfocus-spec SDK, Cobra CLI, zerolog, gRPC, pluginsdk (001-plugin-init-recorder)
- Local filesystem for fixture downloads and recorded request output (001-plugin-init-recorder)

### Recent Changes

- 001-plugin-init-recorder: Added Go 1.27.1 + finfocus-spec SDK, Cobra CLI, zerolog, gRPC, pluginsdk
