...
## Active Technologies
- Go 1.27.1 + `github.com/spf13/cobra` (CLI), `github.com/spf13/viper` (Config), `github.com/rshade/finfocus-spec` (renamed from `finfocus-spec`) (113-rebrand-to-finfocus)
...
- Go 1.27.1 + `github.com/charmbracelet/lipgloss`, `golang.org/x/term`, `github.com/spf13/viper` (existing config), `github.com/spf13/cobra` (existing CLI) (001-cli-budget-alerts)
- Local filesystem (`~/.finfocus/config.yaml`) (001-cli-budget-alerts)
- Go 1.27.1 + `github.com/charmbracelet/bubbletea`, `github.com/charmbracelet/lipgloss`, `github.com/rshade/finfocus/internal/engine` (510-tui-detail-recommendations)

- Markdown, Go 1.27.1 (for code verification) + Jekyll (for docs site), GitHub Pages (010-sync-docs-codebase)
...
- Go 1.27.1 + `github.com/stretchr/testify` (assertions), `net/http/httptest` (mocking) (021-plugin-integration-tests)
- Filesystem (mocked via `t.TempDir()`) (021-plugin-integration-tests)
- Go 1.27.1 + `github.com/spf13/cobra` (CLI), `github.com/spf13/pflag` (023-add-cli-filter-flag)
- Pure Go (no external dependencies for filter logic) (023-add-cli-filter-flag)
- Go 1.27.1 + `github.com/Masterminds/semver/v3` (001-latest-plugin-version)
- Filesystem (`~/.finfocus/plugins/`) (001-latest-plugin-version)
- Go 1.27.1 + github.com/rshade/finfocus-spec v0.4.14, github.com/Masterminds/semver/v3 (112-plugin-info-discovery)
...
Go 1.27.1: Follow standard conventions
...
## Recent Changes
- 510-tui-detail-recommendations: Added Go 1.27.1 + `github.com/charmbracelet/bubbletea`, `github.com/charmbracelet/lipgloss`, `github.com/rshade/finfocus/internal/engine`
- 001-cli-budget-alerts: Added Go 1.27.1 + `github.com/charmbracelet/lipgloss`, `golang.org/x/term`, `github.com/spf13/viper` (existing config), `github.com/spf13/cobra` (existing CLI)
...
- **Docker Images**: Update Docker base images (e.g., `golang:1.24` → `golang:1.27.1`) in documentation examples
...
- **Common Patterns**: Update both specific versions (1.24.10 → 1.27.1) and minimum requirements (Go 1.24+ → Go 1.27.1+)
